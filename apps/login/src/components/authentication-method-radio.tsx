...
export function AuthenticationMethodRadio({
  ...
  return (
    <div className="jayn-auth-method-picker w-full">
      <div className="mx-auto w-full max-w-md">
        <RadioGroup value={selected} onChange={selectionChanged}>
          <Label className="sr-only">Server size</Label>
          <div className="jayn-auth-method-list flex flex-row space-x-4">
            {methods.map((method) => (
              <Radio
                key={method}
                value={method}
                ...
                className={({ focus, checked }) =>
                  ...
                  } jayn-auth-method boder-divider-light dark:border-divider-dark relative flex h-full flex-1 cursor-pointer rounded-lg border px-5 py-4 hover:shadow-lg focus:outline-none dark:hover:bg-white/10`
                }
              >
                <div className="jayn-auth-method-content flex w-full flex-col items-center text-sm">
                  {method === "passkey" && (
                    <svg
                      ...
                      strokeWidth="1.5"
                      stroke="currentColor"
                      className="jayn-auth-method-icon mb-3 h-8 w-8"
                    >
                      <path
                        strokeLinecap="round"
                  ...
                  {method === "password" && (
                    <svg
                      className="jayn-auth-method-icon mb-3 h-8 w-8 fill-current"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                    >
